...
def send(ofp, wid, lname, rname):
	...
	assert os.system('cd %s; ln -s \'%s\' \'%s\'; tar ch \'%s\' | gzip -1 > judge_server.tgz' % (working_dir, os.path.realpath(lname), rname, rname)) == 0
	with open('/%s/judge_server.tgz' % (working_dir), 'rb') as fp: b = fp.read()
...
def work(sid, pid, lng, wid, address, account):
	...
	dtl = ifp.read()
	if dtl:
		with open('%s/../submission/%d-z' % (WORKSPACE, sid), 'wb') as fp: fp.write(dtl)

	print('[' + Fore.MAGENTA + 'Get' + Fore.RESET + '] sid %d time %d space %d score %d' % (sid, cpu, mem, score), file = sys.stderr)

	worker_cursor[wid].execute("UPDATE submissions SET scr=%s, res=%s, cpu=%s, mem=%s WHERE sid=%s" % (score, result, cpu, mem, sid))
	#db.commit()
